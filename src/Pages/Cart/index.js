...
const Cart = () => {
    ...
    useEffect(() => {
        ...
        fetchDataFromApi(`/api/cart?userId=${user?.userId}`).then((res) => {
            if (Array.isArray(res)) {
                setCartData(res); // Ensure cartData is an array
                setselectedQuantity(res?.quantity);
            } else {
                setCartData([]); // Set empty array if API doesn't return an array
            }
        });
    }, []);
    ...
    const selectedItem = (item, quantityVal) => {
        if (chengeQuantity !== 0) {
            ...
            cartFields.rating = item?.rating || 0
            ...
            // Update the item subtotal in the cartData array
            ...
            setCartData(updatedCartData);  // Update cartData with new subtotal
